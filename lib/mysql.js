...
const insertFriend = function (values) {
...
const insertApply = function (values) {
...
const searchUsersByName = function (uname) {
    const _sql = `select users.id uid,users.name uname,users.avator uavator,users.bio ubio,fr.id flist_id
        from users left join friend_rooms fr on users.id=fr.uid where users.name like "${uname}__";`
    return query(_sql);
}
