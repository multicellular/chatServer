...
const users =
    ...
    is_oauth INT NULL DEFAULT 0,
    moment TIMESTAMP NULL DEFAULT CURRENT_TIMESTAMP,
    PRIMARY KEY ( id )
)`;

// 通过第三方授权的用户，通过oauth_id关联user表，即可登录，密码默认‘123456’？
const oauth_users =
...
const insertUser = function (values) {
    const _sql = `insert into users set name=?,password=?,avator=?,is_oauth=?;`
...
const findOauthUser = function (oauth_id) {
    const _sql = `select uid from oauth_users where oauth_id=${oauth_id};`
    return query(_sql);
}

const insertOauthUser = function ({ oauth_id, uid }) {
    const _sql = `insert into oauth_users set oauth_id=${oauth_id},uid=${uid};`
    return query(_sql);
}
...
const findChat = function ({ uid, fuid }) {
    const _sql = `select * from chats where (uid=${uid} and fuid=${fuid}) or (uid=${fuid} and fuid=${uid});`
    return query(_sql);
}
...
module.exports = {
    query, findUserByName, insertUser, updateUserBio, findUserById, findOauthUser, insertOauthUser,/* users操作 */
