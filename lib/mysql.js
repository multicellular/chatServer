...
const insertChatRoom = function ({ uid, fuid }) {
    const _sql = `insert into rooms(uid,fuid) select ${uid},${fuid} from dual where not exists 
    (select uid,fuid from rooms where (uid=${uid} and fuid=${fuid}) or (uid=${fuid} and fuid=${uid}));`
    return query(_sql);
}

const findChatRoom = function ({ uid, fuid }) {
...
module.exports = {
    ...
    insertRoom, findRoom, findSystemRooms, deleteRoom, clearRoomUser, insertChatRoom, findChatRoom,
    userLeaveRoom, listJoinRoom, findRoomsByUser, findUsrsByRoom, updateRoom, findUsrsByChatRoom,
